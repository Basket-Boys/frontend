...
const SERVER = "https://type-tris.herokuapp.com/";
...
class GameWrapperClass extends React.Component {
  ...
  componentDidMount() {
    ...
    getRoomData(this.socket, username, ({ playerID, room, roomPlayers }) => {
      if (!this.state.subscribedResults)
        resultSubscriber(this.socket, playerID, () => this.handleVictory());
      if (!this.state.loadedWords && roomPlayers.length >= 2)
        this.setNewWordList(getWordList());
  ...
  render() {
    ...
    if (this.state.roomPlayers.length < 2)
      return (
        <Center>
          <SpacedColumn spacing={Adjust.spacing.grid}>
            <CText>
              hi, {this.location.state.username}! let's wait for a friend.
            </CText>
    ...
    const mainGame = (
      <Center>
        <SpacedRow spacing={20}>
          ...
          />
          <MistakeMeter value={this.state.mistakeCount} maximum={5} />
          <ScoreBoard comboCount={this.state.comboCount} />
          <OpponentTable
            socket={this.socket}
            gameEnded={this.state.gameEnded}
            playerID={this.state.playerID}
